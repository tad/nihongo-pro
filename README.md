...
- **Study session (early access)** — tap **Start study** after translating a sentence to enter a focused, full-screen study mode with a 25/5-minute pomodoro timer in the top-right. The session begins with a randomized pre-quiz: type the definition and pronunciation of each word (Claude evaluates the definitions; pronunciation accepts hiragana or rōmaji). The kanji pre-quiz follows. Items you nail are skipped; the rest become your study list. Then the session walks you through each kanji (with stroke-order animation and meaning) and each word (with audio and definition), prompting you to write/say each one 5 times on paper. The translation-critique phase ships next.
