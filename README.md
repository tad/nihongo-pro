...
7. Tap the **chart-bar icon** in the top-right to open the **Progress** sheet — at-a-glance counts of unique words and kanji you've encountered, how many you've marked Known, a familiarity distribution bar for each, and your top 25 most-seen words and kanji. Every row is tappable to open the same word/kanji modal you'd get from a sentence, so you can demote a Known item, replay its audio, or jump out to Jisho/Nihongo from there.
8. To change voice or playback speed, tap the gear icon and use the **Speech** section. Tap *Play Sample* to preview your current selection. For better-quality voices, download enhanced/premium variants in iPadOS *Settings → Accessibility → Spoken Content → Voices → Japanese*, then **fully quit Nihongo Pro and reopen it** — new voices won't show up in the picker until the app process restarts.
...
- **Familiarity levels** — set each word and each kanji to **Unknown** (default), **Familiar** (stepping stone, no behavior change), or **Known** via a segmented control. The control lives in three places: the word modal, the kanji modal, and on every card during a study session. Words you mark Known stop showing furigana — and a word whose every kanji you've marked Known also loses its furigana, even if the word itself isn't Known. Marking a word Known does **not** mark its kanji Known (and vice versa) — the two levels are independent until you set them. Known words and kanji are skipped during study-session pre-quizzes so you only drill what you don't already have, and you can also mark items Known mid-quiz (or mid-study) to skip the current card immediately. Known words remain tappable in the sentence so you can demote them back if you forget. Levels persist across app restarts.
- **Study session** — tap **Start study** after translating a sentence to enter a focused, full-screen study mode with a 25/5-minute pomodoro timer in the top-right. The session walks you through, in order: a randomized pre-quiz (type the definition + pronunciation of each word, then the definition of each kanji; Claude grades the meaning and pronunciation accepts hiragana or rōmaji), an active study phase for the items you missed (kanji with stroke-order animation; words with audio playback — both prompt you to write 5 times on paper), and a final translation phase where you type your own English rendering of the original sentence and Claude responds with warm, specific commentary on what worked and what could be improved. If you'd rather skip the pre-quiz, tap **Skip to study** in the toolbar while a pre-quiz card is showing — every non-Known word and kanji from the sentence is queued straight into the study phase.
- **Save & resume** — tap the close-session button while studying to choose between *Continue studying*, *Save & exit* (writes the in-progress session to disk so you can pick up later), or *End session* (discards). When a saved session exists, a **Resume last study session** button appears in the top-left toolbar of the main screen, on every launch, until you tap it or overwrite it by saving a different session. Only one session can be saved at a time. The saved file is cleared automatically when you resume.
