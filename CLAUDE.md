...
- **Clear button:** a destructive-role `.bordered` Clear button sits on the bottom-left of the action row (a `Spacer()` separates it from Translate/Breakdown on the right). `ContentView.clear()` resets `inputText`, `words`, `englishTranslation`, all error/breakdown/loading state, and calls `speechService.stop()` if audio is still playing. Disabled while a translation is in flight or when there is nothing to clear (empty input AND empty results AND no error).
